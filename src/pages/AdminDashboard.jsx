...
// Assurez-vous que ces fichiers existent dans votre structure de dossiers
...
// Composants Shadcn/UI mockés (pour la structure)
...
export default function AdminDashboard() {
    ...
    // État et Logique du Dark Mode
    const [darkMode, setDarkMode] = useState(false); 

    // DARK MODE INIT - Lit la préférence et l'applique
    useEffect(() => {
        const stored = localStorage.getItem("darkMode");
        // Initialisation basée sur localStorage ou la préférence système par défaut
            ...
            <div className="flex-1 flex flex-col md:ml-72 transition-all duration-300 min-w-0">
                ...
                <header className="bg-white dark:bg-gray-800 shadow px-6 py-4 flex justify-between items-center border-b border-gray-300 dark:border-gray-700 sticky top-0 z-50">
                    <div className="flex items-center gap-3">
                        ...
                        {/* 🍔 Bouton Menu Hamburger (AVEC FOND SEMI-TRANSPARENT) 🍔 */}
                        <button
                            onClick={() => setMenuOpen(true)}
                            className={`
                                md:hidden p-2 rounded-lg z-50
                    ...
                    {/* Bouton Dark Mode Semi-Transparent et Flou */}
                    <button
                        onClick={toggleDarkMode}
                        className={`
                            p-2 rounded-full 
                            bg-white/70 dark:bg-gray-700/70 // Fond semi-transparent
