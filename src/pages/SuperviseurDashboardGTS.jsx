...
export default function SuperviseurDashboardGTS() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("dashboard-realtime-gts")
      .on(
        "postgres_changes",
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-900 text-white shadow-2xl transform transition-transform duration-300 md:translate-x-0 md:relative md:flex md:flex-col ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex flex-col h-full">
          ...
          <div className="p-8 text-center bg-slate-950/40 relative">
            <h1 className="text-3xl font-black tracking-tighter italic text-blue-500">GTS</h1>
          ...
          <nav className="mt-6 flex-1 overflow-y-auto no-scrollbar">
            <ul>
              <NavItem id="dashboard" icon={LayoutDashboard} label="Dashboard" active={section === "dashboard"} onClick={() => handleSectionChange("dashboard")} />
              <NavItem id="missions" icon={Truck} label="Missions" active={section === "missions"} onClick={() => handleSectionChange("missions")} />
              <NavItem id="pannes" icon={AlertTriangle} label="Pannes" active={section === "pannes"} onClick={() => handleSectionChange("pannes")} />
              <NavItem id="maintenance" icon={Wrench} label="Maintenance" active={section === "maintenance"} onClick={() => handleSectionChange("maintenance")} />
              <NavItem id="documents" icon={FileWarning} label="Documents" active={section === "documents"} onClick={() => handleSectionChange("documents")} />
              <NavItem id="billing" icon={Receipt} label="Dépenses" active={section === "billing"} onClick={() => handleSectionChange("billing")} />
            </ul>
          </nav>
          
          <div className="p-6 space-y-4 bg-slate-950/50 border-t border-white/5">
            <div className="flex items-center gap-3 p-3 bg-white/5 rounded-2xl">
                <div className="h-10 w-10 rounded-full bg-blue-600 flex items-center justify-center font-black text-white shadow-lg">
                    {userProfile?.display_name?.charAt(0)}
                </div>
                <div className="min-w-0 flex-1">
                    <p className="text-[10px] font-black uppercase text-blue-400">Superviseur</p>
        ...
        <header className="h-16 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between px-8 z-30">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-lg font-bold text-slate-800 dark:text-white italic">
               GTS | {section === "dashboard" ? "Vue d'ensemble" : section.toUpperCase()}
            </h2>
          </div>
          <div className="px-4 py-1 rounded-full text-[10px] font-black tracking-widest uppercase bg-blue-100 text-blue-600">
            ...
            {section === "dashboard" ? (
              <div className="space-y-6 animate-fadeIn">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <StatCard title="Missions Actives" value={stats.missions} icon={Truck} color="blue" onClick={() => handleSectionChange("missions")} />
                  <StatCard title="Flotte Camions" value={stats.camions} icon={ClipboardList} color="green" onClick={() => handleSectionChange("maintenance")} />
                  ...
                  <StatCard title="Alertes Docs" value={stats.docs} icon={FileWarning} color="purple" blink={stats.docs > 0} onClick={() => handleSectionChange("documents")} />
                </div>
