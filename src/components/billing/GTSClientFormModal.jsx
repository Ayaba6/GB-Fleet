// src/components/billing/ClientFormGTSModal.jsx
...
export default function ClientFormGTSModal({ isOpen, onClose, onClientAdded }) {
  ...
  const handleAddClient = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("clients_gts")
        .insert([{ name, address, rccm, ifu, phone, regime_fiscal: regimeFiscal, division_fiscale: divisionFiscale, zone }])
        .select()
        .single();
      ...
      onClientAdded(data); // retourne le client ajouté pour pré-remplir GTSInvoiceForm
      onClose();

      // Réinitialiser les champs
      setName(""); setAddress(""); setRCCM(""); setIFU(""); setPhone(""); setRegimeFiscal(""); setDivisionFiscale(""); setZone("");
      ...
      <div className="bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 rounded-2xl shadow-2xl w-full max-w-md p-6">
        <div className="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 pb-3 mb-4">
          <h2 className="text-xl font-semibold flex items-center gap-2">
            <UserPlus className="text-blue-600" /> Nouveau Client GTS
          </h2>
        ...
        <div className="space-y-4">
          ...
          <input type="text" placeholder="Téléphone" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full p-2 border rounded-md border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800"/>
          <input type="text" placeholder="Régime Fiscal" value={regimeFiscal} onChange={(e) => setRegimeFiscal(e.target.value)} className="w-full p-2 border rounded-md border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800"/>
          <input type="text" placeholder="Division Fiscale" value={divisionFiscale} onChange={(e) => setDivisionFiscale(e.target.value)} className="w-full p-2 border rounded-md border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800"/>
          <input type="text" placeholder="Zone" value={zone} onChange={(e) => setZone(e.target.value)} className="w-full p-2 border rounded-md border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800"/>
        </div>
