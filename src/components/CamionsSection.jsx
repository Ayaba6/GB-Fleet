...
import CamionSummaryModal from "./modals/CamionSummaryModal.jsx";
...
export default function CamionsSection() {
  ...
  const [summaryCamion, setSummaryCamion] = useState(null);
  ...
  const confirmDelete = async () => {
    try {
      const { error } = await supabase
        .from("camions")
        .delete()
        .eq("id", camionToDelete.id);

      if (error) throw error;

      toast({
        title: "Camion supprimé",
        description: `Le camion "${camionToDelete.immatriculation}" a été supprimé.`,
      });
  ...
  // Ouvre le modal résumé
  const handleViewSummary = (camion) => {
    setSummaryCamion(camion);
  ...
  const paginatedCamions = filteredCamions.slice(
  ...
  return (
    <div className="space-y-6 animate-fadeInUp">
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between bg-white/80 dark:bg-gray-800/80 p-4 rounded-xl shadow border border-gray-100 dark:border-gray-700 backdrop-blur-sm">
        ...
        <div className="flex gap-2">
          <Button onClick={exportExcel} variant="outline" className="border-green-500 text-green-600 dark:text-green-400">
            Excel
          </Button>
          <Button onClick={exportPDF} variant="outline" className="border-red-500 text-red-600 dark:text-red-400">
            PDF
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {paginatedCamions.length === 0 ? (
          ...
          paginatedCamions.map((c) => (
            <Card
              key={c.id}
              className="shadow-lg p-4 bg-white/70 dark:bg-gray-800/70 border border-gray-200 dark:border-gray-700 backdrop-blur-sm"
            >
              <div className="flex justify-between items-start">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Button
                    ...
                    className="bg-blue-500/70 dark:bg-blue-600/70 hover:bg-blue-600 dark:hover:bg-blue-700 transition"
                    title="Voir le résumé du camion"
                  >
                  ...
                  <Button
                    ...
                    className="bg-white/50 dark:bg-gray-700/50 hover:bg-white/70 dark:hover:bg-gray-600/60 transition"
                    title="Modifier"
                  >
      ...
      {/* MODALS */}
      {showModal && (
        <CamionModal editingCamion={editingCamion} setShowModal={setShowModal} fetchCamions={fetchCamions} />
      )}

      {summaryCamion && (
        <CamionSummaryModal
          camion={summaryCamion}
