...
export default function AlertesExpirationCardsStyled() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data: camionsData, error: cErr } = await supabase
          .from("camions")
          .select("*");
        ...
        (profilesData || []).forEach((c) => {
          if (c.cnib_expiration)
            alertesList.push({
              cible: c.name,
              type: "CNIB",
              expirationdate: c.cnib_expiration,
              cibleType: "chauffeur"
            });
          ...
          if (c.permis_expiration)
            alertesList.push({
              cible: c.name,
              type: "Permis",
              expirationdate: c.permis_expiration,
              cibleType: "chauffeur"
            });

          if (c.carte_expiration)
            alertesList.push({
              cible: c.name,
              type: "Carte",
              expirationdate: c.carte_expiration,
              cibleType: "chauffeur"
            });
        });

        (camionsData || []).forEach((c) => {
          if (c.cartegriseexpiry)
            alertesList.push({
              cible: c.immatriculation,
              type: "Carte grise",
              expirationdate: c.cartegriseexpiry,
              cibleType: "camion"
            });

          if (c.assuranceexpiry)
            alertesList.push({
              cible: c.immatriculation,
              type: "Assurance",
              expirationdate: c.assuranceexpiry,
              cibleType: "camion"
            });

          if (c.visitetechniqueexpiry)
            alertesList.push({
              cible: c.immatriculation,
              type: "Visite technique",
              expirationdate: c.visitetechniqueexpiry,
              cibleType: "camion"
            });
        });
  ...
  const totalPages = Math.ceil(
    filteredAlertes.length / ITEMS_PER_PAGE
  );
        ...
        {paginatedAlertes.length === 0 ? (
          ...
          paginatedAlertes.map((a) => {
            ...
            return (
              <Card
                key={a.cible + a.type}
                className="shadow-lg p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700"
              >
                <div className="flex justify-between items-start">
                  ...
                  <span
                    className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-sm font-medium ${badge.color}`}
                  >
                    {badge.icon}
                    {badge.text}
                  </span>
