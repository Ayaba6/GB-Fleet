...
const LoadingSpinner = () => (
  <div className="flex justify-center items-center p-10 bg-white dark:bg-gray-900 rounded-xl shadow-lg min-h-[200px]">
...
export default function BillingExpenses() {
  ...
  const StatCard = ({ title, value, colorClass, icon: Icon, description }) => (
    <div className={`bg-white dark:bg-gray-900 shadow-xl rounded-2xl p-6 border-l-4 ${colorClass}`}>
      <div className="flex justify-between items-start">
        <div>
          <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">{title}</h3>
          <p className={`mt-1 text-3xl font-extrabold ${colorClass.replace("border-l-4 ", "").replace("border-", "text-")}`}>{currencyFormatter.format(value)}</p>
        </div>
  ...
  return (
    <div className="p-4 md:p-8 bg-gray-50 dark:bg-gray-950 min-h-screen space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b pb-4 border-gray-200 dark:border-gray-700">
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <Button className="flex items-center gap-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300" onClick={exportExcel}><File size={16}/> Excel</Button>
          <Button className="flex items-center gap-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300" onClick={exportPDF}><FileText size={16}/> PDF</Button>
        </div>
      </div>
        ...
        tabs={[
          {
            ...
            content: loading ? <LoadingSpinner /> : (
              ...
              <div className="p-6 bg-white dark:bg-gray-900 rounded-lg border border-dashed border-gray-300 dark:border-gray-700 text-center">
          ...
          },
          {
            ...
            value: "expenses",
            content: loading ? <LoadingSpinner /> : (
              expenses.length ? <ExpensesList expenses={expenses} refresh={fetchData} camions={camions} /> :
              <div className="p-6 bg-white dark:bg-gray-900 rounded-lg border border-dashed border-gray-300 dark:border-gray-700 text-center">
